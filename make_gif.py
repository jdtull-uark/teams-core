...
def run_model_and_collect_data(
    ...
    """Run the model once and collect all data needed for GIF creation"""
    ...
    print("Model run completed. Data collected.")
    
    return step_data, model_data, grid_size

def create_agent_gif_from_data(step_data, grid_size, filename='agent_movement.gif', fps=2):
    """Create agent movement GIF from collected data"""
    
    print(f"Creating agent movement GIF: {filename}")
    ...
    def animate(frame_num):
        ...
        if agents:
            ...
            ax.scatter(x_coords, y_coords, c=colors, s=sizes, alpha=0.8, edgecolors='black', linewidth=0.5)
    ...
    print(f"Agent GIF saved as {filename}")

def create_metrics_gif_from_data(model_data, filename='metrics_evolution.gif', fps=2):
    """Create metrics GIF from collected data"""
    
    print(f"Creating metrics GIF: {filename}")
    ...
    print(f"Metrics GIF saved as {filename}")

def create_combined_gif_from_data(step_data, model_data, grid_size, filename='combined_simulation.gif', fps=2):
    """Create combined GIF from collected data"""
    
    print(f"Creating combined GIF: {filename}")
    ...
    print(f"Combined GIF saved as {filename}")

def create_all_gifs_from_single_run(
    num_engineers=5,
    initial_tasks=10,
    grid_size=10,
    psychological_safety=0.8,
    contributed_psychological_safety=0.2,
    steps=100,
    fps=2
):
    """Run the model once and create all three GIF types from the same data"""
    
    print("Running model once and creating all GIF types...")
    
    # Run model once and collect all data
    step_data, model_data, grid_size = run_model_and_collect_data(
        num_engineers=num_engineers,
        initial_tasks=initial_tasks,
        grid_size=grid_size,
        psychological_safety=psychological_safety,
    ...
    print("\nCreating GIFs from collected data...")
    
    # Create all three GIFs from the same data
    create_agent_gif_from_data(step_data, grid_size, 'agent_movement.gif', fps)
    create_metrics_gif_from_data(model_data, 'metrics_evolution.gif', fps)
    create_combined_gif_from_data(step_data, model_data, grid_size, 'combined_simulation.gif', fps)
    
    print("\nAll GIFs created successfully from the same model run!")
    print("Files created:")
    ...
    return step_data, model_data

if __name__ == "__main__":
    # Example usage - now runs model only ONCE for all GIFs
    
    # Option 1: Create all three GIF types from single model run
    create_all_gifs_from_single_run(
        num_engineers=5,
        initial_tasks=10,
        steps=100,  # Shorter for faster generation
        fps=3
    )
    
    # Option 2: Run model once, then create specific GIFs
    # step_data, model_data = create_all_gifs_from_single_run(steps=100)
    # 
    # # Then you could create additional variations:
    # create_agent_gif_from_data(step_data, 10, 'agents_slow.gif', fps=1)
    # create_metrics_gif_from_data(model_data, 'metrics_fast.gif', fps=8)